...
#define LOG_MODULE "bacnet/service/h_rd"
    ...
    BACNET_CONFIRMED_SERVICE_DATA *service_data)
{
    ...
    log_debug("ReinitializeDevice!");
    if (service_len == 0) {
        len = reject_encode_apdu(
            &Handler_Transmit_Buffer[pdu_len], service_data->invoke_id,
            REJECT_REASON_MISSING_REQUIRED_PARAMETER);
        log_debug("ReinitializeDevice: Missing Required Parameter. "
                  "Sending Reject!");
        goto RD_ABORT;
        ...
        log_debug("ReinitializeDevice: Sending Abort - segmented message");
        goto RD_ABORT;
    }
    ...
    if (len < 0) {
        ...
        log_debug("ReinitializeDevice: Sending Abort - could not decode");
        goto RD_ABORT;
    }
    ...
    if (rd_data.state >= BACNET_REINIT_MAX) {
        ...
        if (Device_Reinitialize(&rd_data)) {
            ...
            log_debug("ReinitializeDevice: Sending Simple Ack!");
        } else {
            len = bacerror_encode_apdu(
                &Handler_Transmit_Buffer[pdu_len], service_data->invoke_id,
                SERVICE_CONFIRMED_REINITIALIZE_DEVICE, rd_data.error_class,
                rd_data.error_code);
            log_debug("ReinitializeDevice: Sending Error");
        }
    }
RD_ABORT:
    pdu_len += len;
    ...
    if (len <= 0) {
        log_err("ReinitializeDevice: Failed to send PDU");
    }
